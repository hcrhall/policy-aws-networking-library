#  Ensure that no security groups allow ingress from 0.0.0.0/0 to port 3389
This policy enforces recommendations for configuring security-related aspects of the default Virtual Private Cloud (VPC) in AWS and blocks any `aws_security_group` or `aws_security_group_rule` resource configuration that will provide unfettered connectivity to remote console services on TCP port 3389.
